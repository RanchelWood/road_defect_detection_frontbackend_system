...
- [ ] Implement Milestone 2 image inference MVP (completed: auth API + frontend auth screens; remaining: model registry, image upload, YOLO inference, annotated output, history persistence).
...
- Observation: bcrypt 5.x caused passlib runtime failure during password hashing tests on this environment.
  Evidence: Backend test run failed until `bcrypt==4.0.1` was pinned.

- Observation: Frontend build failed initially due UTF-8 BOM in package.json generated by shell writes.
  Evidence: Vite PostCSS loader raised JSON parse error until files were rewritten as UTF-8 without BOM.
...
- Decision: Pin bcrypt==4.0.1 in backend dependencies for passlib compatibility on current environment.
  Rationale: Avoids auth hashing failures seen with bcrypt 5.x during Milestone 1 test execution.
...
Expected backend check:
    ...
    HTTP 200
    {"success":true,"data":{"status":"ok"},"meta":{"request_id":"<uuid>","timestamp":"<iso8601>"}}
...
Plan change note (2026-03-22 / Codex): Executed Milestone 1 scaffolding with runnable health/auth flow, backend tests (5 passed), frontend build validation, and dependency compatibility fixes.