...
| Image Inference Model Family Filter (3D) | `GET /models` | Disable family/model controls while model list loads | Show grouped model options by runtime-discovered engine family (`All` + known engines like `RDDC2020/ORDDC2024/GRDDC2022`) | Show model loading error | If selected family has no models, show safe fallback notice |
...
- Inference model selection must support dynamic engine-family filtering derived from `/models`, grouped options by engine, and safe fallback when a previously selected model is unavailable under the active family filter.
